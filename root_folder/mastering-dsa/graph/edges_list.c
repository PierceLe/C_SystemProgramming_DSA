...
int main() {
    ...
    int*** edges_list = (int***)malloc((n_vertex + 1) * sizeof(int**));
    for (int i = 0; i <= n_vertex; i++) {
        edges_list[i] = (int**)malloc(0);
    }
    for (int i = 0; i <= n_edge; i++) {
        printf("Please enter vertices u, v and its weight: ");
        int u, v, w;
        scanf("%d %d %d", &u, &v, &w);
        edges_list[u] = (int**)realloc(edges_list[u],sizeof(edges_list[u]) * sizeof(int*) + sizeof(int*));
        ...
        edges_list[v] = (int**)realloc(edges_list[v],sizeof(edges_list[u]) * sizeof(int*) + sizeof(int*));
    }
    ...
    for (int i = 1; i <= n_vertex; i++) {
        for (int j = 0; j < sizeof(edges_list[i]) / sizeof(int*) - 1; j++) {
            free(edges_list[i][j]);
        }
        free(edges_list[i]);
    }
